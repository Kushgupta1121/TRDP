    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                t4 = time_sync()
                outputs[i] = tracker_list[i].update(det.cpu(), im0,frame_idx)
                t5 = time_sync()
                dt[3] += t5 - t4
                    ...
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if save_txt:
                            ...
                            with open(txt_path + '.txt', 'a') as f:
                                ...
                                '''
                                f.write(str(int(frame_idx + 1))+','+str(int(id)) +','+ str(int(bbox_left*2)) +','+  # MOT format
                                                               str(int(bbox_top*2)) +','+ str(int(bbox_w*2)) +','+ str(int(bbox_h*2))+','+ str(round(conf,5)) +','+ str(-1) +','+ str(-1)+','+ str(-1) +'\n')
